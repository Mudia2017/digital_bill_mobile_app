...
class _RegisterState extends State<Register> {
  ...
  registration(
    ...
    try {
        ...
        if (userProfile['isSuccess'] == false) {
          ...
          if (userProfile['errorMsg'].containsKey('email')) {
            serviceProvider.popWarningErrorMsg(context, 'Error',
                userProfile['errorMsg']['email'][0].toString());
          ...
          resp = {
            ...
            'otp_secret_key': userProfile['otp_secret_key'],
            'otp_valid_date': userProfile['otp_valid_date'],
            'otp_code': userProfile['otp_code'],
            'isSuccess': userProfile['isSuccess'],
            'statusCode': isEmail['statusCode'],
            'errorMsg': isEmail['errorMsg'],
          };
        }
      } else {
        // CALL THE DIALOG TO ALLOW USER PERFORM OPERATION ON THE UI
      ...
      // CALL THE DIALOG TO ALLOW USER PERFORM OPERATION ON THE UI
      serviceProvider.hudLoadingEffect(context, false);
      resp = {'isSuccess': false, 'errorMsg': error};
    }
    return resp;
  }
                                    ...
                                    if (responseData != null) {
                                      if (responseData['isSuccess'] == true) {
                                        ...
                                        if (responseData['statusCode'] == 200) {
                                          ...
                                          // OPT WAS NOT SENT TO THE EMAIL PROVIDED
                                          bool isAcknowledge = await serviceProvider
                                              .popWarningErrorMsg(
                                                  context,
                                                  'Error',
                                                  'Registration was successful.\nWe were unable to send OTP code to your email!\nEnsure you have a good interent connection and regenerate it on the next page.');
                                          if (isAcknowledge) {
                                            Navigator.of(context)
                                                .pushNamedAndRemoveUntil(
                                                    RouteManager.otpVerify,
